function [Forecasts,lambda1vec,lambda2vec,lambdalagvec,nu0i,lambdaivec,garchparams,c,accrates] = IGSampler(Yraw,p,nu0,nu0lag,ssq0,ssq0lag,ssq0contemp,nu0contemp,S0,S1,h)

[Traw, M] = size(Yraw); % Data dimensions
X = [ones(Traw-p,1) getLags(Yraw,p)]; % Matrix of lags and ones
K = width(X);
Y = Yraw(p+1:end,:);
A_prior = zeros(M*p+1,M); 
a_prior = A_prior(:); % Prior means of vectorized coefficients   

% Get variances from fitted AR(1)s for scaling 
sigma_sq = zeros(M,1);
    for i = 1:M
   ...
   lambda_inter=10000; % Shrinkage factor of intercept
   lambda_cont=ssq0contemp;

   intind=zeros(K,M); % indicator for intercept
   intindscale=zeros(K,M); % indicator for intercept multiplied with inverse of own scaling
   otherind=zeros(K,M); % indicator for cross variables
   otherindscale1=zeros(K,M); % indicator for cross variables multiplied with inverse of own equation scaling
   otherindscale2=zeros(K,M); % indicator for cross variables multiplied with other equation scaling
   lagindicator=zeros(K,M); % lag number
...
% Space for MCMC results and initilizations
S=S0+S1;
lambda1vec=zeros(S,1); % Global shrinkage
lambda2vec=zeros(S,1); % Cross variable shrinkage
lambdacontvec=zeros(S,1); % Contemporaneous shrinkage
lambdalagvec=zeros(S,p); % Lag shrinkage
lambdaivec=zeros(S,length(a_prior)); % Local shrinkage
...
ssq1=((a_draw-a_prior)'*V_i_inv*(a_draw-a_prior)+nu02*ssq0(2))/nu1; % Inverse of mean of Inverse lambda2
lambda2=1/gamrnd(nu1/2,(ssq1*nu1/2)^-1,1); 
...
ssq1=((a_draw-a_prior)'*V_i_inv*(a_draw-a_prior)+nu01*ssq0(1))/nu1;
lambda1=1/gamrnd(nu1/2,(ssq1*nu1/2)^-1,1);
...
ssq1=((a_draw-a_prior)'*diag(V_i_inv)*(a_draw-a_prior)+nu0lag*ssq0lag(l))/nu1;
lambdalagvec(s,l)=1/gamrnd(nu1/2,(ssq1*nu1/2)^-1,1);
end
...
sigma_garch=zeros(size(Y,1),M); % space for GARCH variances
for j=1:M
Xtilde=Y;
Xtilde(:,1:j)=[];
u=Y(:,j)-[X(:,:) Xtilde(:,:)]*[A_draw(:,j);A0_draw(j,j+1:end)']; % Error from current draw (each equation)

thetaold=garchparams(j,:,s-1); % Garch parameters from last draw

if s<100
    propscale=[0.4482    0.0145   -0.4244
    0.0145    0.3948    0.1107
   -0.4244    0.1107    0.6669]; % Initial scale of proposal variance (manually tuned)
    propvar=propscale;
else
    propscale=cov(log(squeeze(garchparams(j,:,1:s-1))'));
    c(s,j+1)=min(100,exp(log(c(s-1,j+1))+1/(s-99)^0.4*(mean(accrates(1:s-1,j+1))-0.25)));
    propvar=c(s,j+1)*propscale+0.0001*eye(3); % Adaptive proposal variance
end
thetastar=exp(log(thetaold)+mvnrnd(zeros(1,3),propvar)); % Proposal draw
if sum(thetastar(2:3))>1
    acc=0; % Check stationarity
else
...
A_draw=zeros(K,M); % Draws of intercept and lag parameters
A0_draw=eye(M); % Draws of contemporaneous parameters
U=zeros(M,1); % Errors of current draws (for later use)
...
if s<100
    propscale=0.0369; % Manually tuned
    propvar=propscale;
else
    propscale=var(log(nu0i(1:s-1)));
    c(s,1)=min(100,exp(log(c(s-1,1))+1/(s-99)^0.4*(mean(accrates(1:s-1,1))-0.25)));
    propvar=c(s,1)*propscale; % Adaptive proposal variance
end

nu0istar=exp(log(nu0i(s-1))+randn(1)*sqrt(propvar)); % Proposal draw
...
if mod(s,100)==0
    s % Output the number of draws each 100 draws
end
